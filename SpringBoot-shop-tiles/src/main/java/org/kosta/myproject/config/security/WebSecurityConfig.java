...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {		
		 ...
		 */
		http.authorizeRequests() .antMatchers("/", "/home", "/myweb/**","/imgUpload/**","/views/**" ,"/user/**","/myboard/**") .permitAll().anyRequest().authenticated();
		...
		http.formLogin().loginPage("/loginForm") // 로그인 폼이 있는 url
				...
				.defaultSuccessUrl("/home",true) // 로그인 성공 후 이동할 url , true 는 로그인 후 이동할 경로를 고정 
